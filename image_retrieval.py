...
def get_top_k_img_paths(scores, opts):
    top_k_img_ids = torch.topk(scores.squeeze(), opts.top_k).values
    # get flickr30k ids
    prefix_pattern = re.compile(r"flickr30k_0*")
    suffix_pattern = re.compile(r"\.npz")

    ids = [suffix_pattern.sub("", prefix_pattern.sub("", top_k)) + ".jpg" for top_k in top_k_img_ids]
...
def main(opts):
    ...
    return get_top_k_img_paths(scores, opts)
...
def run_img_retrieval(model, dataloader):
    ...
    assert all_score.size() == (1, dataloader.dataset.all_img_ids)
